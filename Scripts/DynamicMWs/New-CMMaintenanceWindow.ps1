...
.PARAMETER patchTuesday
    If this is used date will based on patch tuesday. If used without -nextmonth or -currentmonth Maintenace Windows will be created for every month.
.PARAMETER currentmonth
	Limits the creation of Maintenace Windows to one which will be the current month.
.PARAMETER nextmonth
...
#$CollelectionSettings = Get-WmiObject -class SMS_CollectionSettings -Namespace \\$siteserver\root\sms\site_$($SiteCode) | Where-Object {$_.CollectionID -eq "$($CollectionID)"}
...
$class_SMS_ServiceWindow.psbase.Path ="\\$siteserver\ROOT\SMS\Site_$($SiteCode):SMS_ServiceWindow"
...
####################################
$descriptiondate = (Get-Date ($MWstartTime))
	$descriptionday = $descriptiondate.ToShortDateString()
	$descriptiontime = $descriptiondate.ToShortTimeString()
	$MaintenanceWindowDescription = "Occurs on " + "$descriptionday" + " at" + " $descriptiontime"
