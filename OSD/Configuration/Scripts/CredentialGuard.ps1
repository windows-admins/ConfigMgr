...
.NOTES
    ...
    1.0.3 - (2017-08-16) Script updated to not require the task sequence environment, can be run from MDT or standalone. [Modified by: Gabriel Bennett]
...
Begin {
      # Construct TSEnvironment object
        try {
            ...
            $LogFilePath = Join-Path -Path $Script:TSEnvironment.Value("_SMSTSLogPath") -ChildPath "EnableCredentialGuard.log"
        }
        catch [System.Exception] {
            Write-Warning -Message "Unable to construct Microsoft.SMS.TSEnvironment object, using %TEMP% path."
            # Determine log file location
            $LogFilePath = Join-Path -Path $env:TEMP -ChildPath "EnableCredentialGuard.log"
        }
}
Process {
    # Functions
